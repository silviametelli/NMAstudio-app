...
app.layout = html.Div(
    ...
    html.Div([html.Div(   # wind speed
                 [html.Div([html.H6("WIND SPEED (MPH)", className="graph__title")]),
                  dcc.Graph(id="wind-speed", figure=dict(layout=dict(plot_bgcolor=app_color["graph_bg"],
                                                                     paper_bgcolor=app_color["graph_bg"]))),
                  dcc.Interval(id="wind-speed-update", interval=int(GRAPH_INTERVAL), n_intervals=0)],
                  className="two-thirds column wind__speed__container"),
              html.Div(
                      [html.Div(  # histogram
                           [html.Div([html.H6("WIND SPEED HISTOGRAM", className="graph__title")]),
                            html.Div([dcc.Slider(id="bin-slider", min=1, max=60, step=1, value=20,
                                                 updatemode="drag",
                                                 marks={20: {"label": "20"},
                                                        40: {"label": "40"},
                                                        60: {"label": "60"}})],
                                      className="slider"),
                            html.Div([dcc.Checklist(id="bin-auto",
                                                    options=[{"label": "Auto", "value": "Auto"}],
                                                    value=["Auto"],
                                                    inputClassName="auto__checkbox",
                                                    labelClassName="auto__label"),
                                      html.P("# of Bins: Auto", id="bin-size", className="auto__p")],
                                      className="auto__container"),
                            dcc.Graph(id="wind-histogram",
                                      figure=dict(layout=dict(plot_bgcolor=app_color["graph_bg"],
                                                              paper_bgcolor=app_color["graph_bg"])))],
                          className="graph__container first"),
                          # wind direction
                          html.Div([html.Div([html.H6("WIND DIRECTION", className="graph__title")]),
...
def gen_wind_speed(interval):
    ...
    layout = dict(plot_bgcolor=app_color["graph_bg"],
                  ...
                  yaxis={"range": [min(0, min(df["Speed"])),
...
def gen_wind_direction(interval):
    ...
    traces_scatterpolar = [{"r": [0, val, val, 0], "fillcolor": "#084E8A"},
    ...
    layout = dict(height=350,
                  ...
                  font={"color": "#fff"},
                  autosize=False,
                  polar={"bgcolor": app_color["graph_line"],
...
def gen_wind_histogram(interval, wind_speed_figure, slider_value, auto_state):
    ...
    traces_scatter = [{"line_dash": "dash", "line_color": "#2E5266", "name": "Average"},
                      {"line_dash": "dot", "line_color": "#BD9391", "name": "Median"},]

    scatter_data = [dict(type="scatter",
